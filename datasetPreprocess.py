...
images = [file for file in glob.glob('dataset_noisy/no-yawn/*.jpg')]
...
for (i, image) in enumerate(images_noYawn):
    ...
    for f in rects:
        ...
        cv2.imwrite("dataset/no-yawn/" + str(i) + ".jpg", crop_image)
...
images = [file for file in glob.glob('dataset_noisy/yawn/*.jpg')]
images_noYawn = [image.replace("\\", "/") for image in images]
for (i, image) in enumerate(images_noYawn):
    # load the input image, resize it, and convert it to grayscale
    image = cv2.imread(image)
    ...
    for f in rects:
        ...
        pad = 1
        crop_image = image[miny - pad: maxy + pad, minx - pad: maxx + pad]
        cv2.imwrite("dataset/yawn/" + str(i) + ".jpg", crop_image)
